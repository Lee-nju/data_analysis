...
# 信息存储的文件路径
file_cons_save = '../cons_model_infos3.csv'
...
def count_space_and_combs(model, model_paras, cons=''):
    ...
    return [u.count(nvars=para_sum), compute_valid_combs(2, u, model_paras), compute_valid_combs(3, u, model_paras), compute_valid_combs(4, u, model_paras), compute_valid_combs(5, u, model_paras), compute_valid_combs(6, u, model_paras)]
...
def cons_basic_info():
    ...
    fileHeader = ['Name', 'Parameter', 'Constraint', 'Constrained Parameter', 'Raw', 'Valid', 'Proportion', '2-way',
                  '3-way', '2-way(valid)', '3-way(valid)', '4-way(valid)', '5-way(valid)', '6-way(valid)']
    ...
    for index in range(0, len(files), 2):
        ...
        # if f_model_path != 'apache-casa-2-way.model' and f_model_path != 'gcc-casa-2-way.model':
        #     continue
        # if f_model_path != 'Storage3-casa-2-way.model':
        #     continue
        # 打印时间和模型
        print(datetime.datetime.now().strftime(ISOTIMEFORMAT) + ' -- cons ' + str(
            int(index / 2)) + ': ' + f_model_path + ' is running!')

        if f_model_path != 'Services-casa-2-way.model' and f_model_path != 'Storage3-casa-2-way.model' and f_model_path != 'Storage5-casa-2-way.model' and f_model_path != 'grep-casa-2-way.model' and f_model_path != 'sed-casa-2-way.model':
            continue
...
# para_num = [2,3,4,2,6,5]
# print(compute_raw_combs(3, para_num))
